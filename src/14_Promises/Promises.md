...
## 8. Catch, try-catch версия для async-await

1. Когда делаем длинную цепочку `then`, в конце должен быть как минимум один `catch`. 
2. При этом, если в цепочке `then` мы встроили `catch` между `then`, то в `catch` кроме обработки ошибки мы можем вернуть некие данные, чтобы не разрушилась вся цепочка `then`. 
3. Соответственно `catch` тоже возвращает промис.
4. При использовании `async\await` обработка ошибок превращается в классическую обработку ошибок для синхронного кода с использованием блоков `try\catch\finally`. То есть какая бы ошибка в последовательных `await`-ах не произошла, мы ее перехватим в блоке `catch`.

## 8. Про async\await
Если с правой стороны от `await` промис, то движок дождется, когда она за-`resolve`-ится. Если же там просто синхронный код, то просто сразу возвращает эти данные.

 Любая, даже пустая асинхронная функция возвращает промис. То есть эта функция вернет промис, который будетза-`resolve`-лен тем значением, которое эта функция возвращает. Это можно использовать при создании промисификации.
 
> **Теория**
>
>- [Promise](https://learn.javascript.ru/promise)
>- [Промисы](https://learn.javascript.ru/promise-basics)
>- [Promise API](https://learn.javascript.ru/promise-api#promise-resolve-reject)
